...
import { underlyingAssetName } from "../utils/underlyingAssetNames";
...
export const OptionsExplorerScreen: React.FC = () => {
  const { activeSymbol, setActiveSymbol } = useAppState();
  ...
  useEffect(() => {
    if (activeSymbol && activeSymbol.trim()) {
      setSymbol(activeSymbol.trim().toUpperCase());
    }
  }, [activeSymbol]);
  ...
  const loadChain = useCallback(
    async (overrideSymbol?: string) => {
      let s = (overrideSymbol ?? symbol).trim().toUpperCase();
      // If the input is empty but we have an activeSymbol, fall back to that
      if (!s && activeSymbol?.trim()) {
        s = activeSymbol.trim().toUpperCase();
      }
      if (!s) {
        setError("Enter an underlying symbol (e.g. AAPL) before loading the chain.");
        setHasSearched(false);
        return;
      }
      if (!overrideSymbol) setActiveSymbol(s);
      if (overrideSymbol) setSymbol(s);
      ...
      try {
        ...
        if (list.length > 0 && hasIpc) {
          const pv = list.filter((c) => c.right === "put").reduce((sum, c) => sum + (c.volume ?? 0), 0);
          const cv = list.filter((c) => c.right === "call").reduce((sum, c) => sum + (c.volume ?? 0), 0);
          const po = list.filter((c) => c.right === "put").reduce((sum, c) => sum + (c.openInterest ?? 0), 0);
          const co = list.filter((c) => c.right === "call").reduce((sum, c) => sum + (c.openInterest ?? 0), 0);
          if (cv > 0) {
    ...
    [symbol, expiryFrom, expiryTo, hasIpc, loadPcHistory, setActiveSymbol]
  );

  // When user selects a different asset from the sidebar while on Options, reload chain for that asset (same date range)
  useEffect(() => {
    if (!activeSymbol?.trim() || !hasSearched) return;
    loadChain(activeSymbol.trim().toUpperCase());
  }, [activeSymbol]);
  ...
  const importFlatFile = useCallback(async () => {
    ...
    try {
      ...
      if (res?.ok && res.data) {
        ...
        const files = res.data.filesImported;
        const skipped = res.data.filesSkipped;
        let msg = `Imported ${n} P/C rows`;
        if (files != null && files > 0) msg += ` from ${files} file${files === 1 ? "" : "s"}`;
        msg += ".";
        if (skipped != null && skipped > 0) msg += ` (${skipped} file${skipped === 1 ? "" : "s"} skipped)`;
        if (res.data.errors?.length) msg += ` (${res.data.errors.length} warnings)`;
  ...
  const downloadFromS3 = useCallback(async () => {
    if (!hasIpc) {
      setImportMessage("Run the app in Electron to use Download from S3.");
      return;
    }
    setImportMessage("Downloading from S3…");
    ...
    try {
      ...
      if (res?.ok && res.data) {
        ...
        if (n === 0 && days === 0) {
          setImportMessage(
            "No options data found for this date range. Check S3 credentials in Settings, and that the dates are past trading days with options flat files on your plan."
          );
        } else {
      ...
      setImportMessage(`Download failed: ${String(e)}`);
    } finally {
  ...
  return (
    <div>
      ...
      <p style={{ color: "#94a3b8", marginBottom: 24 }}>
        ...
        {" "}(<a href="https://massive.com/docs/flat-files/quickstart#aws-s3-cli" target="_blank" rel="noopener noreferrer" style={{ color: "#93c5fd" }}>AWS S3 CLI</a>).{" "}
        Configure S3 credentials in Settings, then use <strong>Download from S3</strong> (date range) to fetch options day-aggregates, or import a CSV manually.
      </p>
      ...
      {hasIpc && (
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
            ...
            >
              {rangeDownloadLoading ? "Downloading…" : "Download from S3"}
            </button>
          </div>
          {importMessage && (
            <div
              style={{
                ...
                borderRadius: 8,
                background:
                  importMessage.startsWith("Imported") || importMessage.startsWith("Downloaded")
                    ? "#1e3a5f"
                    : importMessage.includes("S3 Forbidden") || importMessage.includes("Import historical P/C")
                      ? "#78350f"
                      : "#7f1d1d",
                color: "#e2e8f0",
                fontSize: 14
          ...
          <p style={{ fontSize: 12, color: "#64748b", marginTop: -8, marginBottom: 16 }}>
            No S3 / flat-file access on your Polygon plan? Use <strong>Import historical P/C</strong> with a CSV file instead.
          </p>
              ...
              {error.includes("No options API key") ? (
                <>
                  ...
                  <p style={{ margin: "8px 0 0", fontSize: 14 }}>
                    Open <strong>Settings</strong>, enter your Polygon.io API key in the
                    &quot;Polygon.io&quot; field, and click <strong>Save</strong>.
                  </p>
                </>
              ) : error.includes("403") ? (
                <>
          ...
          {/* Put/Call & Sentiment – always visible so users know it exists */}
          {contracts.length === 0 ? (
            <div
              style={{
                marginTop: 24,
            ...
            >
              <p style={{ margin: 0, fontSize: 14, color: "#94a3b8" }}>
                <strong style={{ color: "#cbd5e1" }}>Put/Call &amp; Sentiment</strong>
              </p>
              <p style={{ margin: "8px 0 0", fontSize: 13, color: "#64748b" }}>
                Enter a symbol and click <strong>Load chain</strong> to see Put/Call volume ratio and sentiment vs history.
                Use <strong>Download from S3</strong> or <strong>Import historical P/C</strong> to add historical data for the heatmap.
              </p>
            </div>
          ) : null}
          ...
          {contracts.length > 0 && (() => {
            ...
            const underlyingDisplayName = underlyingAssetName(symbol.trim().toUpperCase());
            return (
              <>
                <div
                  style={{
                    ...
                    if (stats.lookbackN < 5) {
                      ...
                      return (
                        <p style={{ margin: "12px 0 0", fontSize: 12, color: "#64748b" }}>
                          {n > 0
                            ? `You have ${n} day${n === 1 ? "" : "s"} of P/C history. Need at least 5 for the heatmap. Use Download from S3 with more dates.`
                            : `No P/C history for this symbol. Enter the symbol, set Download from S3 date range to past trading days, click Download from S3, then Load chain.`}
                        </p>
                      );
                    }
                    return (
                ...
                <p style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 600, color: "#e2e8f0" }}>
                  {underlyingDisplayName} ({symbol.trim().toUpperCase()}) option chain
                </p>
                <p style={{ margin: "0 0 8px", fontSize: 12, color: "#64748b" }}>
                  Greeks and bid/ask require a Polygon plan that includes options quotes. If you see — for these, your plan may not include them.{" "}
                  <a href="https://polygon.io/pricing" target="_blank" rel="noopener noreferrer" style={{ color: "#93c5fd" }}>Check plans</a>.
                  Last trade is shown when bid/ask are unavailable.
                </p>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid #334155" }}>
                    <th style={{ textAlign: "left", padding: "6px 8px", color: "#94a3b8" }}>Underlying</th>
                    ...
                    <th style={{ textAlign: "right", padding: "6px 8px", color: "#94a3b8" }}>Last</th>
                ...
                <tbody>
                  {contracts.map((c) => (
                    <tr key={c.id} style={{ borderBottom: "1px solid #1e293b" }}>
                      <td style={{ padding: "6px 8px" }}>{underlyingDisplayName}</td>
                      ...
                      <td style={{ padding: "6px 8px", textAlign: "right" }}>
                        {c.bid > 0 || c.ask > 0 ? c.bid.toFixed(2) : "—"}
                      </td>
                      <td style={{ padding: "6px 8px", textAlign: "right" }}>
                        {c.bid > 0 || c.ask > 0 ? c.ask.toFixed(2) : "—"}
                      </td>
                      <td style={{ padding: "6px 8px", textAlign: "right" }}>
